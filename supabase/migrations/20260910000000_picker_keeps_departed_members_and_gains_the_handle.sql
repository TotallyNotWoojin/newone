-- Restores 20260908090000 and adds the @handle to the payload in one piece.
--
-- 20260909230000 released a migration held since Sep 7, and that file carried a
-- snapshot of this function from Sep 7 - so applying it reverted
-- 20260908090000, which had taught the picker to offer people who had left the
-- group. Searching the picker by @username then found nobody, caught by
-- groups-05 on the device suite.
--
-- The lesson is in the held-migrations README now: a held file is a snapshot,
-- and anything that touches the same function after it is written must be
-- merged in before it ships.
    ...
    where membership.organization_id = p_organization_id
      ...
      and not exists (
        ...
        where current_member.organization_id = p_organization_id
          ...
          -- Only an ACTIVE member is already in the group. Somebody who left or
          -- was removed keeps their row, and without this the picker hid them
          -- for ever, so a group could never take them back (found Sep 7 2026,
          -- the read half of the fix that 20260908070000 made on the write half).

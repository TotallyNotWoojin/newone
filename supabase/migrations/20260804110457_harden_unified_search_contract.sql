...
-- Human-approved corrections are separate governed records. Index them
-- independently so unified search can use the effective approved translation
-- without treating an obsolete machine rendering as current.
create index if not exists translation_corrections_approved_body_search_idx
  on public.translation_corrections using gin (
    to_tsvector('simple', private.normalize_search_text(corrected_body))
  )
  where status = 'approved';

create or replace function private.bff_search_v2_impl(
...
begin
  ...
  with searchable as (
    ...
    select
      ...
      left(case
        when 'original' = any(v_match_sources) and message.body_search @@ v_query
          then message.body
    ...
    where 'messages' = any(v_types)
      ...
      and (
        ('original' = any(v_match_sources) and message.body_search @@ v_query)
...
drop function public.bff_search(
  uuid, uuid, uuid, text, text[], text, integer, uuid, timestamptz, timestamptz
);
...
comment on function public.bff_search(
  uuid, uuid, uuid, text, text[], text, integer, uuid, timestamptz, timestamptz, text[]
) is 'Authorization-trimmed unified search. Message-source filters never expand conversation visibility.';

-- The translation resolution names the speaker.
--
-- The model translating a message received nothing but the text. The father
-- writing "아빠 kyle" about himself came back as "her dad Kyle", and "써머리"
-- (summary, spelled as it sounds) as a person called Summer (owner reports,
-- Sep 14 2026). The resolution now carries the sender's display name as
-- sender_display_name; the worker forwards it as a "Speaker:" line. The rest
-- of the function is the live definition, unchanged.
create or replace function private.bff_resolve_translation_job_for_egress_impl(p_worker_id uuid, p_job_id bigint, p_provider text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
begin
  ...
  -- The sender's display name rides along so the model knows who is talking:
  -- the father's "아빠 kyle" read as "her dad Kyle", and "써머리" (summary) as a
  -- person called Summer (owner reports, Sep 14 2026). Null when the profile
  -- is gone; the worker treats that as no name.
  if found then
    select profile.display_name into v_sender_name
    from public.profiles profile
    where profile.user_id = v_message.sender_user_id;
  end if;

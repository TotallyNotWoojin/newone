-- No duplicate groups. A group whose live members are exactly the ones being
-- picked -- the creator included -- is handed back instead of creating another,
-- with its own outcome ('already_exists') so the app can offer to open it.
-- Consumer groups only; workplace realms may repeat a member set deliberately.

-- Defect AD: adding username to the candidate rows broke every shipped client,
-- whose parsers accept an exact key set. Keep the @username MATCHING from
-- 20260906070000 but return the original row shape; the handle can join the
-- payload once every client tolerates it.

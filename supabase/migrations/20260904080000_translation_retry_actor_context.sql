-- Translation retry after a failed detection: run inside the command preamble.
--
-- 20260904060000/070000 checked conversation membership before the command
-- preamble had installed the actor context, so under the API's service-role
-- call auth.uid() was null and every retry answered 403. The failed-detection
-- branch now prepares the command first (idempotent replay included), checks
-- membership with the actor installed, does its work, and finishes the
-- command with the same response shape the existing path records.

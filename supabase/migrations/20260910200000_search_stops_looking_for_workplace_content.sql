-- Search stops looking for announcements and handoffs, correctly this time.
--
-- The first attempt removed the two union arms by line number and took the
-- "numbered" CTE with them, so every search returned a 503. Redone by finding
-- the searchable CTE, splitting its body on top-level "union all", and
-- dropping the arms whose FROM is a table that no longer exists -- with a
-- check that the function's CTE list (searchable, page, numbered) is identical
-- before and after.

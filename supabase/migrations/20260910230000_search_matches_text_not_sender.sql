-- A search matches what a message says, not who said it.
--
-- With no explicit match_sources, the messages arm defaulted to
-- ['original', 'translation', 'sender', 'attachment_filename'], so searching a
-- person's name returned: the person, every chat with them, and then every
-- message they had ever sent -- which buried the messages that actually
-- contained the word. The name is already the People section directly above.
--
-- 'sender' stays selectable for a messages-only search that asks for it; it is
-- just no longer assumed. The caller cannot opt out on a mixed search either:
-- passing match_sources is rejected unless the search is messages-only, so the
-- default is the only place this can be fixed.
...
begin
  ...
  -- A plain search matches what a message says, not who said it. 'sender' is
  -- still selectable, but it is no longer a default: searching a name returned
  -- the person, every chat with them, and then every message they had ever
  -- sent, which buried the messages that actually contained the word (owner,
  -- Sep 10 2026). Their name is already the People section above.
  select array_agg(distinct requested_source order by requested_source)
    into v_match_sources
  from unnest(coalesce(
    p_match_sources,
    array['original', 'translation', 'attachment_filename']::text[]
  )) requested_source;

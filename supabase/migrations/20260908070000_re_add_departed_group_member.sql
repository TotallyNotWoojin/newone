-- Removing someone from a group and adding them back was impossible.
--
-- A departure keeps the membership row and marks it 'left' or 'removed', but
-- the add path inserted blindly, so the primary key rejected the second join
-- with a conflict the app showed as "The request conflicts with the current
-- state." The insert now revives a departed row as an ordinary member — the
-- exact shape private.validate_conversation_member_write already permits for a
-- rejoin — and still refuses when that person is already an active member.

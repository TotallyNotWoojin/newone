-- Defect P: system events in the bootstrap timeline must always carry target_user_id.
-- jsonb_strip_nulls removed the key when the event had no target (avatar changed, etc.),
-- which the client rejected as an invalid system event and stalled bootstrap on a spinner.
-- Build system_event outside the stripped object so the key is present with a null value.

...
-- The public Auth API is intentionally reachable with the publishable key.
-- Newone's OTP gateway adds invitation and abuse controls, but a caller can
-- still address GoTrue directly. This Auth hook makes token issuance itself
-- fail closed unless the principal is currently entitled to a workspace or is
-- the exact subject of an unexpired, single-use invitation awaiting redemption.
-- It applies to initial OTP verification, MFA upgrades, and token refreshes.
...
revoke all on function public.hook_newone_custom_access_token(jsonb)
  from public, anon, authenticated, service_role;
grant usage on schema public to supabase_auth_admin;
grant execute on function public.hook_newone_custom_access_token(jsonb)
  to supabase_auth_admin;

comment on function public.hook_newone_custom_access_token(jsonb) is
  'Supabase Auth custom-access-token hook: permits JWT issuance only for a current canonical organization membership or an exact live invitation principal.';

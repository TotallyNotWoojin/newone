-- Translation retry after a failed language detection.
--
-- A provider outage that outlasted the worker's retries left a message with
-- language_detection_state = 'failed' and its translation rows 'blocked'
-- ("Translation unavailable by policy"), and public.bff_enqueue_translation
-- refused to retry it because detection was not 'completed'. The person
-- reading the message had no way out. Owner requirement (Sep 3 2026): a retry
-- must exist, must not be spammable, and one failed message must stay its own
-- isolated case.
--
-- This wrapper adds the failed-detection path in front of the existing logic:
-- membership is required, two rate limits apply (5 retries per actor per 5
-- minutes across all messages, 3 per message per hour), the detection state
-- returns to 'pending', failed or blocked translation rows return to 'queued'
-- (a detection completion enqueues queued rows), and a fresh detection job is
-- posted under a retry-specific dedupe key so the completed original never
-- shadows it. Everything else still flows through the existing implementation.

-- The push carries the unread count for the app icon.
--
-- Owner's father, Sep 19 2026: the mark on the Android app icon never shows.
-- The app only ever set the icon number from inside a running process
-- (setBadgeCountAsync in notification-navigation.native.ts), and the push
-- itself carried no badge. That cannot work on either platform once the app
-- is closed: iOS shows only what the push's aps.badge says, and Android
-- launchers show the number the notification itself carries (Samsung a
-- count, Pixel a dot) -- the legacy ShortcutBadger write the in-app call
-- falls back to is a no-op on every modern launcher. So the resolver now
-- hands each delivery the reader's unread total and the worker sends it as
-- the Expo message's badge.
--
-- The whole function is restated, not patched by text transform; the only
-- change from 20260910220000 is the new 'badge_count' key. The CTE list
-- (eligible_users, candidate_devices, inserted_attempts, attempt_rows, page,
-- numbered) is unchanged and no join was replaced. The v2..v5 layers above
-- this function extend each delivery with || and jsonb_set, so the key passes
...
begin
  ...
  select
    coalesce(jsonb_agg(jsonb_build_object(
      ...
      -- The number on the app icon. It travels with the push because the
      -- phone cannot work it out on its own: iOS only shows what aps.badge
      -- says, and Android launchers show the number a notification carries.
      -- Every message still unread across the reader's chats, counted exactly
      -- as bff_bootstrap_messaging_state_impl counts unread_count for the chat
      -- list, so the icon and the list never disagree. Per user, not per
      -- device: two phones show the same number. Uncapped here; the worker
      -- caps it the way the app itself does (99) before it leaves.
      'badge_count', (
        select count(*)
        from public.conversation_members badge_member
        left join public.conversation_preferences badge_preference
          on badge_preference.organization_id = badge_member.organization_id
         and badge_preference.conversation_id = badge_member.conversation_id
         and badge_preference.user_id = badge_member.user_id
        left join public.conversation_read_cursors badge_cursor
          on badge_cursor.organization_id = badge_member.organization_id
         and badge_cursor.conversation_id = badge_member.conversation_id
         and badge_cursor.user_id = badge_member.user_id
        join public.messages unread
          on unread.organization_id = badge_member.organization_id
         and unread.conversation_id = badge_member.conversation_id
        where badge_member.organization_id = v_job.organization_id
          and badge_member.user_id = numbered.user_id
          and badge_member.status = 'active'
          and coalesce(badge_preference.is_hidden, false) is false
          and unread.deleted_at is null
          and unread.available_at <= now()
          and (badge_member.history_visible_from is null
            or unread.created_at >= badge_member.history_visible_from)
          and unread.id > coalesce(badge_cursor.last_read_message_id, 0)
          and unread.sender_user_id <> numbered.user_id
          and not exists (
            ...
            where visibility.organization_id = unread.organization_id
              ...
              and visibility.user_id = numbered.user_id
          )
      ),

-- Translation retry after a failed detection: revive finished translation jobs.
--
-- Proved on hosted (2026-09-04 04:36 UTC): after 20260904080000 the retry
-- was accepted and detection re-ran, but the translation row stayed 'queued'
-- because its job 'translation:<id>' had already completed and the detection
-- completion's enqueue only bumped it. Revive those jobs to pending (as the
-- existing retry path does) so the same worker pass claims them.
...
begin
  ...
  if found and v_detection_state = 'failed' then
    -- The command preamble records idempotency and installs the actor's
    -- context (auth.uid() and session) that the membership check reads; the
    -- API calls this function with the service role and explicit actor args.
    ...
    -- The translation jobs for those rows already ran once (completed or
    -- failed) under 'translation:<id>'; the detection completion re-enqueues
    -- under the same key and would only bump a finished job. Revive them the
    -- way the existing retry path does, so the pass that finishes detection
    -- claims them straight away.
    ...
    return private.finish_bff_command_internal(
      ...
      p_idempotency_key, p_request_sha256,
      jsonb_build_object('translation_id', v_translation_id, 'status', 'queued', 'retried', true),
      202
    );
  end if;

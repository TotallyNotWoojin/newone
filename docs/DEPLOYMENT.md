...
This runbook covers the independent Gist Expo/Supabase product. It intentionally contains no ChatGPT Sites or D1 deployment path.
...
| Development | Synthetic only | Shared-device, Auth, Realtime, Storage, and bounded integration testing | Current Free `newone` project; reserved origin `https://dev.gist.invalid` |
...
- nine versioned Edge Functions exist in source; six are active in development: `newone-api`, `newone-auth`, `newone-bootstrap`, `newone-read`, `gist-outbox-worker`, and `newone-maintenance-worker`;
...
- `https://dev.gist.invalid` is a reserved non-routable Auth origin, not a deployed website or an acceptable employee redirect.
...
EXPO_PUBLIC_SUPPORT_CONTACT_URL=https://support.example/gist
...
NEWONE_ALLOWED_WEB_ORIGINS=https://app.gist.example
NEWONE_PUBLIC_APP_URL=https://app.gist.example
...
NEWONE_ACCESS_COOKIE_NAME=__Host-gist_access
NEWONE_REFRESH_COOKIE_NAME=__Host-gist_refresh
...
NEWONE_OPENROUTER_APP_NAME=Gist
...
The development project currently has the custom access-token hook and session hardening enabled, and `newone-auth` is deployed. Hosted core artifact `gist-e2e-20260805t073237z-bbb833d0` passed real password sessions, installation binding, AAL1 privileged denial, TOTP/AAL2, one-time invitation redemption, replay denial, and rate-limit exhaustion. That does not make employee sign-in ready: the Auth origin is the reserved `https://dev.gist.invalid`, and a real web domain, Turnstile secret/hostname policy, custom SMTP, redirect proof, provider delivery, and signed-device/browser evidence remain required.

- disable open email/phone signup;
- apply migrations before Auth configuration, enable the versioned
  `hook_gist_custom_access_token` hook, and verify initial OTP, MFA upgrade,
  ...
  `config.toml` exist;
- configure verified redirect URLs for `gist://auth/callback` and each web origin;
...
- pass each challenge token once as Supabase Auth's `captchaToken`. GoTrue owns the Siteverify exchange; Gist must not call Cloudflare Siteverify a second time or attempt to reuse the single-use token;
- record that the widget requests Turnstile action `workplace_sign_in`, but the current Supabase GoTrue CAPTCHA integration does not let Gist configure or prove an expected-action comparison. Treat the action as non-enforced metadata unless the Auth provider adds that control or a separately reviewed single-owner verification design replaces this flow;
...
- probe the public Auth endpoint directly with a synthetic unrelated principal
  ...
  membership blocks refresh without relying on the Gist Edge gateway;
...
- approve a named human-verification procedure and trained recovery-manager roster. Gist records only the keyed digest of the external reference; never place identity documents or the raw reference in the app, database, request logs, or release evidence;
...
On hosted Free projects created after the provider's June 3, 2026 change, custom Auth email templates cannot be used with the default SMTP service. Configure a custom SMTP provider or approved Send Email Auth Hook before relying on Gist's code-only invitation/sign-in template. Default SMTP is not a production employee-delivery path.
...
- The development deployment currently contains six active functions: `newone-api`, `newone-auth`, `newone-bootstrap`, `newone-read`, `gist-outbox-worker`, and `newone-maintenance-worker`. The AI, attachment-scan, and push-receipt workers are not deployed. The Bootstrap gateway remains active but fails closed without its temporary secret; the hosted-run secret was removed after cleanup.
...
The secret-free, gitignored artifact `tests/hosted/.artifacts/gist-e2e-20260805t073237z-bbb833d0.json` records 18 passing real steps against hosted Supabase Auth, Postgres/RLS, the four core gateway functions, and private Realtime. It covers two organization bootstraps, bound sessions, TOTP/AAL2, invitation redemption/replay denial, contacts, direct/group conversations, multilingual persistence, private-channel delivery and tenant denial, idempotent replay/conflict, raw-table non-disclosure, and real `201`/`429` rate-limit exhaustion. Guarded database and Auth cleanup completed. A later dry-run reported six active functions and zero existing organizations/users, and the ephemeral Bootstrap secret was absent.
...
- Production web must be built as a static Expo export and served on a Gist/company-controlled origin with the same-origin `/api` BFF, TLS, CSP/Turnstile directives, no-store employee-data responses, and no ChatGPT dependency. The reserved `https://dev.gist.invalid` origin is intentionally non-routable and does not satisfy this requirement. Another approved host must reproduce and re-test the BFF trust/header contract; uploading only the static files is insufficient for secure web Auth.

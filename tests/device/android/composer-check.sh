...
"$ADB" -s $DEV shell pm clear com.totallynotwoojin.gist >/dev/null
# The common flows expect the app already open (the iOS harness warms it up).
"$ADB" -s $DEV shell monkey -p com.totallynotwoojin.gist -c android.intent.category.LAUNCHER 1 >/dev/null 2>&1
sleep 15

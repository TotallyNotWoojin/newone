...
export async function run(ctx) {
  ...
  if (!open.uiOk) return;
  const photo = await ctx.step({ id: 'viewer-02-photo', title: 'Send a photo from the library', device: dev, flow: 'media/photo-library.yaml', expected: 'photo bubble with preview', screen: 'conversation', timeoutMs: 240_000 });

...
export async function run(ctx) {
  ...
  if (!open.uiOk) return;
  // The conversation already holds a photo from an earlier run; if not, send one.
  await ctx.step({ id: 'viewer-02-photo', title: 'Ensure a photo is in the thread', device: dev, flow: 'showcase/ensure-photo.yaml', env: { PEER: peer }, expected: 'a photo bubble', screen: 'conversation', timeoutMs: 240_000, optional: true });

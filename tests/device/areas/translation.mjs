...
export async function run(ctx) {
  ...
  const [A, B] = await Promise.all([
    ctx.signup(devA, { label: 'tr_a', displayName: `Sim Avery ${tag}`, language: 'en' }),
    ctx.signup(devB, { label: 'tr_b', displayName: `Sim Beatriz ${tag}`, language: 'es' }),
  ]);
  ...
  const intro = `Hello Beatriz, nice to meet you ${tag}`;
  ...
  const en1 = `The meeting moved to Monday morning, please bring the report ${tag}`;
  ...
  const seen = await ctx.step({
    id: 'trans-02-b-sees-spanish', title: 'B sees the Spanish translation card (TRANSLATION · ES) with the original kept', device: devB, flow: 'translation/see-translation.yaml', env: { TEXT: en1, LANG: 'ES', TIMEOUT: '90000' },
    expected: 'Within 60s: translation card labelled TRANSLATION · ES, ORIGINAL · EN label and the English text still visible', screen: 'conversation', latencyFrom: sentAt,
    serverTruth: async () => {
      const w = await server.waitFor(() => server.messageByBody(convId, `report ${tag}`), (r) => (r?.translations ?? '').includes('es=completed'), { timeoutMs: 90_000 });
      const row = w.row;
      let latency = null;
  ...
  ctx.note({ id: 'trans-02b-latency', title: 'Measured translation latency (send tap → Spanish card visible on B)', status: seen.uiOk ? 'INFO' : 'FAIL', observed: `${Math.round((seen.entry.latencyMs ?? 0) / 100) / 10}s on device (includes B flow startup); server row: ${JSON.stringify(seen.serverResult?.detail ?? null)}` });
  await ctx.observe(devB, { id: 'trans-02c-spanish-render', title: 'Exact Spanish rendering on B', screen: 'conversation' });
  ...
  const es1 = `Perfecto, llevaré el informe impreso el lunes ${tag}`;
  const sentEs = Date.now();
  await ctx.step({ id: 'trans-04-b-sends-spanish', title: 'B replies in Spanish', device: devB, flow: 'chat/send-text.yaml', env: { TEXT: es1 }, expected: 'bubble', screen: 'conversation' });
  await ctx.step({
    ...
    serverTruth: async () => { const w = await server.waitFor(() => server.messageByBody(convId, `lunes ${tag}`), (r) => (r?.translations ?? '').includes('en=completed'), { timeoutMs: 90_000 }); return { ok: w.ok, detail: w.row?.translations }; },
    timeoutMs: 240_000,
  });
  ...
  const en2 = `Second English message while translation is off ${tag}`;
  ...
  await ctx.step({ id: 'trans-11-translations-return', title: 'Translation cards return after switching back on', device: devB, flow: 'translation/see-translation.yaml', env: { TEXT: en2, LANG: 'ES', TIMEOUT: '90000' }, expected: 'TRANSLATION · ES card visible again', screen: 'conversation', timeoutMs: 240_000 });
  ctx.accounts = { A, B };
}

// PEOPLE on two devices: name/@username search, "Message" opens a chat with
// anyone at once (the Sep 2026 social stream removed connect requests and
// message requests for consumers), the recipient simply sees the message, the
// People tab lists the people you have chatted with ("Your people"), a
// display-name change reaches the other side, block → sender's send fails
// visibly → unblock. Server truth on every durable step.
...
export const meta = { id: 'people', devices: 2, title: 'PEOPLE (A/B + C as the stranger)' };
...
export async function run(ctx) {
  ...
  await ctx.step({
    ...
    expected: 'Row with C\'s display name, @username and a "Message" button (no Connect, no "Send message request")', screen: 'people',
  });
  ...
  const firstMessage = await ctx.step({
    ...
    flow: 'people/message-from-result.yaml', env: { TEXT: firstText },
  ...
  await ctx.step({
    ...
    expected: 'B\'s row with a Message button', screen: 'people',
  });
  ...
  await ctx.step({
    ...
    flow: 'people/message-from-result.yaml', env: { TEXT: hello },
  ...
  const live = await ctx.step({
    id: 'people-10-a-sees-friend', title: 'A\'s People tab lists B under "Your people" without relaunch', device: devA,
    flow: 'people/friends-shows.yaml', env: { NAME: B.displayName },
    expected: 'B listed under "Your people" with a Message button within 45s of the first message', screen: 'people',
  });
  if (!live.uiOk) {
    await ctx.step({
  ...
  if (rename.serverResult?.ok) {
    await ctx.step({
      id: 'people-12c-b-sees-new-name', title: 'B sees A\'s new display name on the People row (no relaunch)', device: devB,
      flow: 'people/friends-shows.yaml', env: { NAME: renamed },
      expected: 'People → Your people shows the renamed A within 45s', screen: 'people',
    });
    await ctx.step({
  ...
  await ctx.step({
    id: 'people-13-block', title: 'B blocks A from the People row\'s "…" menu (Manage contact and privacy)', device: devB,
    flow: 'people/block.yaml', env: { NAME: A.displayName },
    expected: '"Blocked" badge replaces the Message button on A\'s row; server member_blocks row', screen: 'people → Contact and privacy',
    serverTruth: async () => {
  ...
  await ctx.step({
    ...
    flow: 'people/unblock.yaml', expected: '"Blocked" badge gone, Message button back; server member_blocks row removed', screen: 'people → Contact and privacy',
    serverTruth: async () => {
  ...
  // people-17-remove-connection: removed — consumer People rows have no

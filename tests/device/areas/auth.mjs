...
//   Returning sign-in (v3.2): email → password step → wrong password refused
//     → right password signs in. Sign out again, then forgot password:
//     recovery code → resend (cooldown + confirmation) → superseded code
//     refused → fresh code → new password saved → signed in, with the
//     recovering session still alive.
...
const NEW_PASSWORD = 'Newone-reset-2026';
...
// The server's "this account has a chosen password" stamp (newone-auth/handler.ts).
async function passwordStamp(server, userId) {
  ...
  return row?.password_set_at ?? null;
}
...
async function signupWithWrongCodeFirst(ctx, device, { username, displayName, mailbox }) {
  ...
  const form = await ctx.step({
    ...
    expected: 'Form accepts email/username/display name/password; "One-time code" screen appears', screen: 'sign-in',
  });
...
export async function run(ctx) {
  ...
  // Returning sign-in (v3.2): email → password. The signup form typed
  // SIGNUP_PASSWORD (common/signup-request.yaml), so the account has one.
  const returning = await ctx.step({
    id: 'auth-07-returning-request', title: 'Returning sign-in ("Sign in" chip): email → Continue → password step', device,
    flow: 'common/returning-request.yaml', env: { EMAIL: activeMailbox.email },
    expected: 'The lookup finds the account; the password field appears (no code screen, no method or channel chips)', screen: 'sign-in (Sign in chip)',
  });
  if (returning.uiOk) {
    await ctx.step({
      id: 'auth-08-wrong-password', title: 'Wrong password is refused with a visible message', device,
      flow: 'auth/wrong-password.yaml', env: { PASSWORD: 'Wrong-password-2026', OBSERVE: '"That email or password is incorrect." and still on the password step' },
      expected: '"That email or password is incorrect."; not signed in', screen: 'sign-in (password step)',
    });
    const sessionsBefore = new Set(userId ? (await server.sessions(userId).catch(() => [])).map((row) => row.id) : []);
    await ctx.step({
      id: 'auth-09-password-signin', title: 'Right password signs in', device,
      flow: 'common/returning-verify.yaml', env: { PASSWORD: SIGNUP_PASSWORD },
      expected: 'Chats appears; server: a new unrevoked session installation row', screen: 'sign-in (password step)',
      serverTruth: async () => {
        if (!userId) return { ok: false, detail: 'no user id resolved after signup' };
        const wait = await server.waitFor(() => server.sessions(userId), (rows) => rows.some((row) => !sessionsBefore.has(row.id) && !row.revoked_at), { timeoutMs: 30_000 });
        return { ok: wait.ok, detail: (wait.row ?? []).map((row) => ({ id: row.id, platform: row.platform, revoked_at: row.revoked_at, new: !sessionsBefore.has(row.id) })) };
      },
    });
  }

  // Forgot password: recovery code (resend + stale code) → new password.
  await ctx.step({ id: 'auth-10-sign-out-again', title: 'Sign out again for the forgot-password road', device, flow: 'common/signout.yaml', expected: 'Sign-in screen', screen: 'settings' });
  const forgotRequest = await ctx.step({ id: 'auth-11-returning-request-2', title: 'Returning sign-in: email → password step again', device, flow: 'common/returning-request.yaml', env: { EMAIL: activeMailbox.email }, expected: 'password step', screen: 'sign-in (Sign in chip)' });
  if (forgotRequest.uiOk) {
    const forgot = await ctx.step({ id: 'auth-12-forgot-password', title: '"Forgot password?" emails a recovery code', device, flow: 'auth/forgot-password.yaml', expected: '"One-time code" screen; the password field is gone', screen: 'sign-in (password step)' });
    if (forgot.uiOk) {
      const at = Date.now();
      const first = await ctx.waitForCode(activeMailbox);
      mailNote(ctx, { id: 'auth-13-forgot-email', title: 'Recovery code arrives', code: first, expected: 'code email within 150s', arrived: `arrived after ${Math.round((Date.now() - at) / 1000)}s`, missing: 'no email arrived' });
      const since = Date.now() - at;
      if (since < 65_000) await sleep(65_000 - since);
      const resend = await ctx.step({
        id: 'auth-14-resend-code', title: 'Resend code: confirmation + disabled link during cooldown', device,
      ...
      mailNote(ctx, { id: 'auth-15-resend-email', title: 'Resent recovery code arrives in the real inbox', code: second,
      ...
      if (first && second && second.code !== first.code) {
        const stale = await ctx.step({
          id: 'auth-16-stale-code', title: 'Superseded (old) code is refused after resend', device,
        ...
        if (!stale.uiOk) code = null; // old code was accepted → already past the code screen (finding recorded)
      } else {
        ctx.note({ id: 'auth-16-stale-code', title: 'Superseded (old) code is refused after resend', status: 'SKIPPED', observed: second ? 'resend reissued the identical code; nothing stale to test' : 'no resent code arrived; testing the first code below instead' });
      }
      if (code) {
        const verified = await ctx.step({
          id: 'auth-17-forgot-verify', title: `Recovery code (${second ? 'resent' : 'original'}) opens "Set a new password"`, device,
          flow: 'auth/forgot-verify.yaml', env: { CODE: code },
          expected: '"Set a new password" screen; nobody is signed in yet', screen: 'sign-in (One-time code)',
        });
        if (verified.uiOk) {
          const stampBefore = userId ? await passwordStamp(server, userId).catch(() => null) : null;
          await ctx.step({
            id: 'auth-18-new-password', title: 'New password saved → signed in (the write keeps the recovering session alive)', device,
            flow: 'auth/new-password.yaml', env: { PASSWORD: NEW_PASSWORD },
            expected: 'Chats; server: app_metadata.newone_password_set_at moved forward and the new session installation is not revoked (defect AF)', screen: 'sign-in (Set a new password)',
            serverTruth: async () => {
              if (!userId) return { ok: false, detail: 'no user id resolved after signup' };
              const wait = await server.waitFor(() => passwordStamp(server, userId), (stamp) => Boolean(stamp) && stamp !== stampBefore, { timeoutMs: 30_000 });
              const live = (await server.sessions(userId).catch(() => [])).filter((row) => !row.revoked_at);
              return { ok: wait.ok && live.length >= 1, detail: { stampBefore, stampAfter: wait.row, liveSessions: live.length } };
            },
          });
        }
      }
  ...
  await ctx.step({
    id: 'auth-19-delete-account', title: 'Delete account (type username, confirm) → back at sign-in', device,
